...
const MAX_PAGES = 6;                 // bound runtime on big multi-page B/Ls
...
async function extractTextFromPdf(pdfPath) {
  ...
  try {
    await execFileP('pdftotext', ['-layout', '-f', '1', '-l', String(MAX_PAGES), pdfPath, txtPath], { timeout: 30000 });
  ...
  const created = [];
  try {
    await execFileP('pdftoppm', ['-png', '-r', '200', '-f', '1', '-l', String(MAX_PAGES), pdfPath, prefix], { timeout: 180000 });
