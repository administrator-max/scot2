...
// Gemini (multimodal) reads the document image/PDF directly — best accuracy on scans.
const GEMINI_MODEL = process.env.GEMINI_MODEL || 'gemini-2.5-flash';
const GEMINI_MAX_BYTES = 12 * 1024 * 1024; // inline_data is request-size bound; bigger -> fall back
function geminiKey() { return process.env.GEMINI_API || process.env.GEMINI_API_KEY || ''; }
...
// Send the raw document (image or PDF) to Gemini's multimodal model — it OCRs and
// extracts structured fields in one call, which is far more robust on scans.
async function extractWithGemini(buffer, mimeType, name) {
  const key = geminiKey();
  if (!key) throw new Error('Gemini key not configured');
  if (buffer.length > GEMINI_MAX_BYTES) {
    throw new Error(`File too large for inline Gemini (${Math.round(buffer.length / 1048576)}MB > 12MB)`);
  }
  const isPdf = /pdf/i.test(mimeType || '') || /\.pdf$/i.test(name || '');
  const inlineMime = isPdf ? 'application/pdf'
    : /png/i.test(mimeType || '') ? 'image/png'
    : /webp/i.test(mimeType || '') ? 'image/webp'
    : /tif/i.test(mimeType || '') ? 'image/tiff'
    : 'image/jpeg';
  ...
  const body = {
    contents: [{
      parts: [
        ...
        { inline_data: { mime_type: inlineMime, data: buffer.toString('base64') } }
      ]
    }],
    generationConfig: {
      temperature: 0,
      responseMimeType: 'application/json',
      thinkingConfig: { thinkingBudget: 0 } // extraction needs reading, not reasoning — keep it fast/cheap
    }
  };
  ...
  const parsed = filterFields(JSON.parse(stripJson(txt)));
  return { method: 'gemini-vision', source: 'gemini', fields: parsed.fields, confidence: parsed.confidence, textPreview: '' };
}

// One call the server uses: Gemini first (when configured), else tesseract/poppler
// + Claude/regex. Always returns { method, source, fields, confidence, textPreview }.
async function processDocument(buffer, mimeType, name) {
  if (geminiKey()) {
    try {
      return await extractWithGemini(buffer, mimeType, name);
    } catch (e) {
      console.warn('[ocr] Gemini failed, falling back to local OCR:', e.message);
    }
  }
  const { text, method } = await extractText(buffer, mimeType, name);
  const parsed = await parseFields(text);
  return {
    method,
...
module.exports = { extractText, parseFields, extractWithGemini, processDocument, FIELD_KEYS };
